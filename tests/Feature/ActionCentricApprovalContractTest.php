...
it('derives approvable action catalog from action attributes when explicit catalog is absent', function (): void {
    ...
    expect($actions)
        ->toHaveKey('profile.update')
        ->toHaveKey('delete')
        ->toHaveKey('fiscal-data.update')
        ->toHaveKey('contacts.update')
        ->toHaveKey('status.change')
        ->and($actions['profile.update'])->toBe('Atualizar perfil')
        ->and($actions['delete'])->toBe('Excluir agência')
